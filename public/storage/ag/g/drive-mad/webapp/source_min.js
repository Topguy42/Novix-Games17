...
Audio.allow = function () {
  Audio.allowed = true; for (let i = 0; i < Audio.postInit.length; i++) { Audio.postInit[i](); }
...
var Storage = {}; Storage.VERSION = 1; Storage.PATH_VERSION = 'com.martinmagni.drivemad/version'; Storage.PREFIX = 'com.martinmagni.drivemad/data/'; Storage.LEGACY_PREFIX = '/'; Storage.stringToHex = function (str) {
  var hex = ''; for (var i = 0; i < str.length; i++) { hex += str.charCodeAt(i).toString(16) + " "; }
  return hex;
}
Storage.arrayToBase64 = function (array) {
  let binaryString = ''; for (let i = 0; i < array.byteLength; i++) { binaryString += String.fromCharCode(array[i]); }
  return window.btoa(binaryString);
}
Storage.base64ToArray = function (base64) {
  const binaryString = window.atob(base64); let array = new Uint8Array(binaryString.length); for (let i = 0; i < binaryString.length; i++) { array[i] = binaryString.charCodeAt(i); }
  return array;
}
Storage.get = function (prefix, path) {
  let base64; try { base64 = localStorage.getItem(prefix + path); } catch (e) { return null; }
  if (base64 === null)
...
Storage.put = function (prefix, path, data) {
  let base64; try { base64 = Storage.arrayToBase64(data); } catch (e) { console.error(`Failed to put path '${path}' with data '${Storage.stringToHex(data)}'`); console.error(e); return; }
  try { localStorage.setItem(prefix + path, base64); } catch (e) { }
}
Storage.migrate0 = function () {
  let old_version; try { old_version = localStorage.getItem(Storage.PATH_VERSION); } catch (e) { return; }
  if (old_version !== null)
  ...
  for (let i = 0; i < len; i++) {
    let key; try { key = localStorage.key(i); } catch (e) { continue; }
...
Storage.sync = function () {
  let len; try { len = localStorage.length; } catch (e) { return; }
  const path_ptr = _malloc(Storage.PATH_MAX); let data_cap = 1024; let data_ptr = _malloc(data_cap); for (let i = 0; i < len; i++) {
    let key; try { key = localStorage.key(i); } catch (e) { continue; }
...
var startGameAttempts = 0; var tryStartGameTimeout = null; function tryStartGame() {
  console.log("tryStartGame()", gameReadyToStart, pokiInited, theDomLoaded, postRunDone); if (gameReadyToStart) { return; }
...
var Module = {
  locateFile: function (path, prefix) {
    ...
    updateLoadProgress();
  }, totalDependencies: 0, monitorRunDependencies: function (left) { this.totalDependencies = Math.max(this.totalDependencies, left); }, postMainLoop: function () { Audio.queuedata(); }, onRuntimeInitialized: function () { tryStartGame(); }
}; var notifications = []; var webViewIframe = null; var storedScripts = []; var webviewDomLoaded = false; var fsSyncStatus = ""; async function isUrlFound(url) { try { const response = await fetch(url, { method: 'HEAD', cache: 'no-cache' }); return response.status === 200; } catch (error) { return false; } }
...
function webViewOpen(path) {
  try {
    let arr = readLocalFile(path); let html = new TextDecoder("utf-8").decode(arr); if (webViewIframe == null) {
